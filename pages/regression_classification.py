...
if st.sidebar.button('Predict Prices'):
    if target in df.columns and features:
        ...
        if search_type in ["GridSearch", "RandomizedSearch"]:
            ...
            n_iter = len(search.cv_results_['params']) if search_type == "GridSearch" else n_iter_search
            for i in range(n_iter):
                progress = (i + 1) / n_iter
                progress_bar.progress(progress)
                status_text.text(f"Completed iteration {i + 1}/{n_iter}")
                console_output.text_area("Console Output", value=f"Iteration {i + 1}/{n_iter} complete.\n", height=200)
        else:
            search.fit(X_train, y_train)

...
st.markdown('''
This application predicts prices using various regression models. Choose a dataset from the dropdown menu for demonstration.
''')
...
if origin_col in df.columns and destination_col in df.columns:
    ...
    st.sidebar.warning(f"Columns {origin_col} and/or {destination_col} not found in the dataset.")
...
features = st.sidebar.multiselect('Select features to use:', df.columns.drop([target] + datetime_columns), default=df.columns.drop([target] + datetime_columns).tolist())
encode_categorical = st.sidebar.radio("Encode categorical variables:", ["Frequency Encoding", "Target encoding"], index=0)

model_type = st.sidebar.selectbox('Choose a regression model:', ['Ridge', 'Lasso', 'Random Forest Regressor', 'Decision Tree Regressor', 'XGBoost Regressor', 'AdaBoost Regressor', 'Gradient Boosting Regressor'])
...
search_type = st.sidebar.radio("Search Type", ["None", "RandomizedSearch", "GridSearch"], index=0)
n_iter_search = st.sidebar.number_input("Number of iterations (for RandomizedSearch)", min_value=1, value=10)
cv_folds = st.sidebar.number_input("Number of CV folds", min_value=2, value=3)

# Filtering based on origin and destination
origin = st.sidebar.selectbox(f'Select {origin_col.replace("_", " ")}:', df[origin_col].unique())
destination = st.sidebar.selectbox(f'Select {destination_col.replace("_", " ")}:', df[destination_col].unique())
filtered_df = df[(df[origin_col] == origin) & (df[destination_col] == destination)]

# Check if filtered_df is empty
if filtered_df.empty:
    st.error("No data available for the selected origin and destination. Please select different options.")
else:
    ...
    param_distributions = {
        'Ridge': {'model__alpha': np.linspace(0.01, 10.0, 100), 'model__solver': ['auto', 'svd', 'cholesky', 'lsqr', 'sparse_cg', 'sag', 'saga']},
        'Lasso': {'model__alpha': np.linspace(0.01, 10.0, 100), 'model__max_iter': [100, 500, 1000, 5000, 10000]},
        'Random Forest Regressor': {'model__n_estimators': [100, 200, 300, 400, 500], 'model__max_depth': [None, 10, 20, 30], 'model__min_samples_split': [2, 5, 10], 'model__min_samples_leaf': [1, 2, 4]},
        'Decision Tree Regressor': {'model__max_depth': [None, 10, 20, 30], 'model__min_samples_split': [2, 5, 10], 'model__min_samples_leaf': [1, 2, 4]},
        'XGBoost Regressor': {'model__n_estimators': [100, 200, 300, 400, 500], 'model__learning_rate': [0.01, 0.1, 0.2, 0.3], 'model__max_depth': [3, 5, 7, 9], 'model__subsample': [0.6, 0.8, 1.0], 'model__colsample_bytree': [0.6, 0.8, 1.0]},
        'AdaBoost Regressor': {'model__n_estimators': [50, 100, 150, 200], 'model__learning_rate': [0.01, 0.1, 0.5, 1.0], 'model__loss': ['linear', 'square', 'exponential']},
        'Gradient Boosting Regressor': {'model__n_estimators': [100, 200, 300, 400, 500], 'model__learning_rate': [0.01, 0.1, 0.2, 0.3], 'model__max_depth': [3, 5, 7, 9], 'model__min_samples_split': [2, 5, 10], 'model__min_samples_leaf': [1, 2, 4]}
    }
    ...
    if st.sidebar.button('Predict Prices'):
        if target in df.columns and features:
            ...
            if len(X) < 100:
                st.warning("Insufficient data, generating synthetic data to ensure enough samples.")
                if y.nunique() <= 1:
            ...
            while len(X_resampled) < 10:
                X_resampled, y_resampled = resample(X, y, replace=True, n_samples=10, random_state=42)
                numeric_X_resampled = X_resampled.select_dtypes(include=[np.number])
                numeric_X_resampled += np.random.normal(0, 0.01, numeric_X_resampled.shape)
            ...
            if search_type == "GridSearch":
                search = GridSearchCV(pipeline, param_distributions[model_type], cv=cv_folds, verbose=3)
            elif search_type == "RandomizedSearch":
                search = RandomizedSearchCV(pipeline, param_distributions[model_type], n_iter=n_iter_search, cv=cv_folds, random_state=42, verbose=3)
            else:
                search = pipeline
            ...
            if len(features) != len(importance):
                st.warning(f"Length mismatch: features({len(features)}) and importance({len(importance)})")
                feature_importance_df = pd.DataFrame({'Features': features[:len(importance)], 'Importance': importance}).sort_values(by='Importance', ascending=False)
            else:
                feature_importance_df = pd.DataFrame({'Features': features, 'Importance': importance}).sort_values(by='Importance', ascending=False)

            feature_importance_df = feature_importance_df.sort_values(by='Importance', ascending=True).head(len(importance))
            ...
            st.error("Please select a valid target variable and at least one feature for prediction.")

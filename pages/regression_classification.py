...
def load_data(filename):
    return pd.read_csv(filename).select_dtypes(
        exclude=['datetime', 'datetimetz'])  # Exclude date columns
...
features = st.sidebar.multiselect('Select features to use:',
                                  ...
                                  default=df.columns.drop(target).tolist())
...
categorical_transformer = TargetEncoder() if encode_categorical == "Target encoding" else Pipeline(
    [
        ...
        ('freq_enc', FunctionTransformer(lambda X: pd.DataFrame(X).apply(
            lambda x: x.map(x.value_counts(normalize=True)), axis=0).values,
                                         validate=False))])
...
if st.sidebar.button('Predict Prices'):
    if target in df.columns and features:
        ...
        if search_type == "GridSearch":
            search = GridSearchCV(pipeline, param_distributions[model_type],
        ...
        feature_importance_df = pd.DataFrame(
            {'Features': features, 'Importance': importance}).sort_values(
        ...
        st.error(
            "Please ensure the dataset includes the target column and select at least one feature.")
